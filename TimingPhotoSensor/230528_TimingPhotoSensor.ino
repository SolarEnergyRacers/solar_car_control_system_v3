...
// unsigned long ulActTimeActRound = 0;  // aktuell fortlaufende Zeit der Runde
String strActTimeActRound = "";
...
void loop(void){
  ...
  if (xEs1 == HIGH & xEs1_first == LOW){
    ...
    Serial.println("Start erste Runde 12h-Fahrt");
    writeToSDCard("--------Start erste Runde 12h-Fahrt--------");
  ...
  // Zeitmessung --------------------------------------
 // strActTimeActRound = String((millis())/1000 + " s");
    ...
    writeToSDCard("Runde " + String(iRoundCounter) + ": " + strTimeLastRound);      // write on sd card
    ...
    Serial.print(", Zeit: ");
    Serial.println(strTimeLastRound);             
  }
