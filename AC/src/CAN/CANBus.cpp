...
bool CANBus::writePacket(uint16_t adr, uint16_t data0, uint16_t data1, uint16_t data2, uint8_t data3, uint8_t data4) {
  ...
  if (canBus.verboseModeCanOut)
    console << fmt::format("{:3x}: {:08x} <== {:02x} - {:02x} - {:02x} - {:01x} - {:01x}\n", adr, data, data0, data1, data2, data2, data4);
...
string CANBus::print_raw_packet(CANPacket packet) {
  return fmt::format("C{}-{}-[{:02d}|{:02d}] CAN.PacketId=0x{:03x}-R-data: {:064x}\n", xPortGetCoreID(),
                     esp_timer_get_time() / 1000000, availiblePackets(), getMaxPacketsBufferUsage(), packet.getId(), packet.getData_ui64());
}
